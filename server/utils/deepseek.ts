...
export async function login_deepseek_via_account(account: any, account_filename: string) {
    ...
    try {
        ...
        if (!data?.data?.biz_data?.user?.token) {
            console.error(`[login_deepseek_via_account] Invalid response: ${JSON.stringify(data)}`);
        ...
        console.error(`[login_deepseek_via_account] Exception: ${e}`);
        throw e;
    }
}
...
export async function create_session(request: RequestWithState, max_attempts = 3): Promise<string | null> {
    ...
    while (attempts < max_attempts) {
        ...
        try {
            ...
            // Only try to parse JSON if response is OK or we suspect it contains error info
            let data: any = {};
            try {
                data = await response.json();
            } catch (jsonErr) {
                 console.error(`[create_session] JSON parse error: ${jsonErr}`);
            }
            ...
            console.warn(`[create_session] Failed: code=${data.code}, msg=${data.msg}`);
            
            // If we are using a config token (managed account) and it failed, maybe try another account or re-login?
            // The original logic tried to rotate accounts here if it failed.
            // For now, let's just increment attempts.
            // TODO: Implement account rotation/retry logic fully if needed
            
            attempts++;
            
        } catch (e) {
            console.error(`[create_session] Exception: ${e}`);
            attempts++;
        }
    }
    return null;
...
export async function get_pow_response(request: RequestWithState, max_attempts = 3): Promise<string | null> {
    let attempts = 0;
    // Ensure this path is correct relative to the running process
        ...
        try {
            ...
            if (data.code === 0) {
                ...
                try {
                     ...
                     console.error(`[get_pow_response] PoW calculation exception: ${e}`);
                     answer = null;
                }
                ...
                console.warn(`[get_pow_response] Failed code=${data.code}`);
                attempts++;
            }

        } catch (e) {
            console.error(`[get_pow_response] Exception: ${e}`);
            attempts++;
        }
    }
    return null;
